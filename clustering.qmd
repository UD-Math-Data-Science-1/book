...
In this chapter we will look at clustering. 

## Introduction {.unnumbered}

The goal of clustering is to assign each feature vector a number from 1 to $k$, where $k$ is much smaller than the number of samples. More formally:
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_g0aquvfp&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_phu9p9l7%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.0" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::

The clusters $C_1,\ldots,C_k$ divide the samples into $k$ nonoverlapping subsets. Depending on the algorithm, the number of clusters $k$ may be imposed (i.e., as a hyperparameter) or determined automatically. 
...
While classification only requires us to separate different classes of examples, clustering is more specific and more demanding: samples in a cluster need to be more like each other, or the "average" cluster member, than they are like members of other clusters. We should expect that edge cases will look ambiguous, and there may be isolated examples that don't fit well into any cluster.

### Toy examples {.unnumbered}

In order to get a feeling for the algorithms, we will apply them to three illustrative "toy" datasets in two dimensions. Working in two dimensions allows us to visualize the results easily, something that is its own challenge when working in higher dimensions.
...
When a distance metric is available, we intuitively expect similarity to be inversely related to distance. There are various precise ways to transform distance to similarity when needed, but we will not use them. Instead, we will take the proposition to "maximize similarity" as equivalent to "minimize distance."

::: {.callout-note}
A practical advantage of working with similarity rather than distance that we will *not* explore is that small values of similarity can be rounded down to zero. Such rounding has negligible effect on the results, but it can create big gains in execution time and memory usage. 
:::
...
:::{.column-margin}
<div style="max-width:400px"><div style="position:relative;padding-bottom:71.25%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_ib6r7f6v&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_qusl9muf%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.1.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

Note that $D_{ii}=0$ and $D_{ji}=D_{ij}$. The latter property is important throughout linear algebra.

::::{#def-cluster-symmetric}
An $n\times n$ matrix $\bfA$ in which $\bfA_{ij}=\bfA_{ji}$ for all $i,j$ is called **symmetric**.
::::
...
:::{.column-margin}
![](_media/memes/symmetry.jpg){.lightbox  width=200px fig-align="center"}
:::

::: {.callout-tip}
In many problems, $n$ is much larger than $d$, so the distance matrix can be faster and easier to work with. Some clustering algorithms use only $\mathbf{D}$ and allow you to supply that in lieu of the feature vectors. 
:::

The scikit-learn function `pairwise_distances` computes a distance matrix efficiently using any of the common vector norms.
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_ol2razoi&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_0yqv9mzw%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.1.2" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
We might represent a text document by a vector of the number of occurrences of certain keywords. In sentiment analysis, these would be terms such as *happy*, *excited*, *sad*, *angry*, *confused*, and so on. For the sake of argument, suppose three documents have the feature matrix
$$
\bfX = 
\begin{bmatrix}
...
::::{.column-margin}
![](_media/memes/angler_distance.jpg){.lightbox  width=200px fig-align="center"}
::::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_fjvhcqjb&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_2c6h9wnu%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.1.3" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
It turns out that in most practical problems, there is a way out of the mess: the data points are usually not uniformly or randomly distributed. That means that distance-based approaches can be made to work. Often, though, a first step is to find a way to reduce the dimensionality without changing the essential structure of the data, a step that is beyond the scope of this course.

## Performance measures

Before we start generating clusterings, we will discuss how to evaluate them. A clustering is essentially a partitioning of the samples into disjoint subsets. I will use some nonstandard terminology that makes the definitions a bit easier to state and read.

::::{#def-cluster-buddies}
We say that two distinct sample points in a clustering are **buddies** if they are in the same cluster, and **strangers** otherwise. 
::::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_7ods82y2&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_jfk5aup4%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.2.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_ypfju76b&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_z2ulfwa1%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.5" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

### Silhouettes

If no reference clustering/classification is available, then the only way we can assess the quality of a clustering is to measure whether samples are more similar to their buddies than to strangers. 
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_jpupxgag&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_gncrozv3%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.2.2" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_o43p6lb3&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_jgi6ofc2%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.7" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_o43p6lb3&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_jgi6ofc2%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.8" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_y47kfwrc&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_z5eyw2yl%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.9" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
<br/>
![](_media/memes/silhouette.jpg){.lightbox  width=200px fig-align="center"}
:::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_vi87asd8&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_mez31qjx%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.3" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
I_2 &= \bigl(2-\tfrac{14}{3}\bigr)^2 + \bigl(5-\tfrac{14}{3}\bigr)^2 + \bigl(7-\tfrac{14}{3}\bigr)^2 = \tfrac{38}{3} \approx 12.67,
\end{split}
$$

for a total inertia of 14.67.
...
The standard iteration is known as **Lloyd's algorithm**. Starting with values for the $k$ centroids, $\bfmu_1, \ldots, \bfmu_k$, each iteration consists of two steps:

:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_zyb6uloq&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_hnt47br6%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.3.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
::: {.callout-note}
Two-phase iterations like Lloyd's algorithm are common in optimization: you start with an approximate answer, compute something based on it, and use that to improve your answer.
:::
...
::::{.column-margin}
![](_media/memes/k-means.jpeg){.lightbox  width=200px fig-align="center"}
::::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_2vl75uac&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_w50jpcoe%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.11" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::

::: {.callout-warning}
In @exm-cluster-kmeans-blobs, we were able to visualize the clusters because the data is two-dimensional. That is not a trivial matter in higher dimensions, and we usually have to rely on scoring methods like silhouette values to guide us.
:::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_c3zy37yk&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_kfhf8pkc%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.13" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_415gg3ez&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_a35pm951%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.4(a)" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_ftn4hgyr&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_tyj3yk2u%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 6.4(b)" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_63xey2da&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_9s0fblo0%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.14" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_2cz7adyp&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_tdncazv0%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.15" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
As you can see, the single linkage was badly confused by the two blobs that nearly run together. The others are good at reproducing the ball-like clusters.
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_5zsztoyl&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_lt9b15gi%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.16" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_7zcbk4s8&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_7xuro6ln%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 6.17" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::
...
::::{#exr-cluster-cosine}
(6.1) Find an example for which the cosine pseudodistance does not satisfy the triangle inequality. That is, find three vectors $\bfx$, $\bfy$, and $\bfz$ such that 
$$
\dist(\bfx,\bfy) > \dist(\bfx,\bfz) + \dist(\bfz,\bfy). 
$$
(Hint: it's enough to play around with some simple vectors in two dimensions. Use vectors for which the angle between is a multiple of $\pi/6$ or $\pi/4$, so that you know the exact values of the cosine.)
::::
...
**(a)** Find the Rand index (*not* the ARI) between clusterings A and B.
...
![Samples for @exr-cluster-inertia.](_media/exer6_5.svg){#fig-exercise6_5}
...
$$
(This means clustering by the colors in @fig-exercise6_5.)
...
$$
(This means clustering by the shapes in @fig-exercise6_5.)

**(c)** It seems obvious that when $z$ is large, we would prefer the clustering in part (a), and when it is close to zero, we would prefer the one in part (b). Find the value of $z$ at which the transition occurs, i.e., when the inertias are equal.
::::
