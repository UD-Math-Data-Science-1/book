...
**recall** 
: How often are actual *positive* cases predicted correctly?

**specificity** 
: How often are actual *negative* cases predicted correctly?
...
A plot is more informative than the table of numbers:
...
::::{#exr-class-dankness}
(2.2) Here is a confusion matrix for a classifier of meme dankness. 
...
::::{#exr-class-flavors}
(2.2) Here is a confusion matrix for a classifier of ice cream flavors. 
...
::::{#exr-class-gini}
(2.3) Find the Gini impurity of this set: 
$$ \{ A, B, B, C, C, C \}.$$
::::

::::{#exr-class-maxgini}
(2.3) Explain why for the binary case $K=2$, @def-class-gini implies that $H(S)$ is maximized when the two classes are equally represented in $S$.
::::

::::{#exr-class-partition}
(2.3) Given $x_i=i$ for $i=0,\ldots,5$, with labels
$$
...
::::{#exr-class-tree}
(2.3) For the decision tree drawn in @exm-class-tree, make predictions for each of the following queries, showing the path taken through the tree for each case.
...
::::{#exr-class-distance}
(2.4) Using 1-norm, 2-norm, and $\infty$-norm, find the distance between the given vectors.
...
::::{#exr-class-norm}
**(a)** (2.4) Prove that for any $\bfu \in \real^d$, $\infnorm{\bfu} \le \twonorm{\bfu}$. 

**(b)** (2.4) Prove that for any $\bfu \in \real^d$, $\twonorm{\bfu} \le \sqrt{d}\, \infnorm{\bfu}$. 
::::

::::{#exr-class-manhattan}
(2.4) Carefully sketch the set of all points in $\real^2$ whose 1-norm distance from the origin equals 1. This is a *Manhattan unit circle*. (Hint: You can consider each quadrant of the plane separately.)
::::

::::{#exr-class-knn1d}
(2.4) Suppose you are training to fit the ground-truth one-dimensional classifier
...
(2.4) Here are blue/orange labels on an integer lattice.
...
::::{#exr-class-knn-roc}
(2.5) Suppose you have written a kNN classifier with $k=5$ to predict whether dad jokes are funny. Here are the votes for 6 test jokes:
