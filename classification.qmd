...
![](_media/memes/no%20fp.jpg){height="420"}
...
If we look down the middle column, for example, we see all the cases where the classifier predicted *Mozart*. In 18 of those cases, that was the correct label, while in $5+2=7$ cases, it was not. The first 18 cases are considered true positives for finding Mozart, while the other 7 are false positives. The precision score for *Mozart* is thus $18/(18+7)$, or 72%.

We could also look at the bottom row, which tells us that out of 26 cases where the true label is *Schubert*, the classifier correctly predicted (true positives) 20 of them, missing (false negatives) on the other 6. The recall score for *Schubert* is thus $20/26$, or about 77%.
::::
...
::::{#def-class-gini}
The **Gini impurity** of sample set $S$ is defined as 
$$
...
::: {.solution}
We can partition into two pieces based on a threshold value $\theta$: $\{x \le \theta\}$ and $\{x > \theta\}$. But there are really only 5 different cases.
...
![](_media/memes/decision%20tree.jpeg){height=420}
...
$$
\begin{split}
\twonorm{\bfu-\bfv} &= \bigl( 1^2 + 0^2 + 2^2 + 2^2 \bigr)^{1/2} = \sqrt{9} = 3, \\ 
\onenorm{\bfu-\bfv} &= 1  + 0 + 2 + 2 = 5, \\ 
...
::::{#exm-class-knn-standard chapter=3 type=💻 description="Pipeline for standardized columns"}
...
::::{#exm-class-roc-simple chapter=3 type=💻 description="ROC curve, quasi-manually"}
...
![](_media/memes/ROC.jpeg){height=380}
...
::::{#exm-class-roc-penguin chapter=3 type=💻 description="ROC curve for the penguins dataset"}
...
![](_media/memes/AUC.jpg){height=360}
...
::::{#exr-class-manhattan}
Carefully sketch the set of all points in $\real^2$ whose 1-norm distance from the origin equals 1. This is a *Manhattan unit circle*. (Hint: You can consider each quadrant of the plane separately.)
::::

::::{#exr-class-triangle}
Three points in the plane lie at the vertices of an equilateral triangle. One is labeled A and the other two are B. Carefully sketch the decision boundary for $k$-nearest neighbors with $k=1$, using the 2-norm.
::::
