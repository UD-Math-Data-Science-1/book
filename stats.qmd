...
:::{#exr-stats-summary}
The following parts are about the sample set of $n$ values $x_1,\ldots,x_n$, where $n>2$, $x_i=0$ for $i=1,\ldots,n-1$, and $x_n=M$ for a positive number $M$. 
...
**(a)** Show that the sample z-scores satsify $\displaystyle \sum_{i=1}^n z_i = 0.$

**(b)** Show that the sample z-scores satsify $\displaystyle \sum_{i=1}^n z_i^2 = n-1.$
:::

:::{#exr-stats-outlier}
For this exercise, apply the 2σ outlier criterion the sample set given in @exr-stats-summary using the sample mean and sample variance. (Again assume that $n>2$ and $M>0$.)

**(a)** Show that regardless of $n$ and $M$, the value 0 is never an outlier.

**(b)** Show that the value $M$ is an outlier if $n \ge 6$, regardless of $M$.
:::
