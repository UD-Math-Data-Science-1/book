...
The pandas function we use to load a dataset is `read_csv`. Here we use it to read a file that is availble over the web:
...
::::{#exm-data-selection chapter=1 description="Selecting rows in a frame" type=💻}
...
::::{#exm-data-time-index chapter=1 description="Using time as an index" type=💻}
Let's reload the dataset using the *DATE* column (which is second in the file) as a `datetime` index:
```{python}
