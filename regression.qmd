...
4. If $\hat{y}_i=\bar{y}$ for all $i$ (i.e., always predict the sample mean), then $R^2=0$.
::::
...
Continuing with @exm-regression-polynomial, we explore the effect of polynomial degree, we split into train and test sets:
...
for deg in range(2,11):
    ...
    MSE = mean_squared_error(y_test, poly.predict(X_test))
...
Here's a LASSO regression fit on the diabetes data:

```{python}
from sklearn.linear_model import Lasso
lass = Lasso(alpha=0.1)
lass.fit(X_train, y_train)
R2 = lass.score(X_test, y_test)
...
```{python}
X_train_reduced = X_train.drop(zeroed, axis=1)
X_test_reduced = X_test.drop(zeroed, axis=1)
```
...
R2 = lm.score(X_test_reduced, y_test)
...
print( pipe.predict(X_train.iloc[:5,:]) )
print("\nprobabilities:")
print( pipe.predict_proba(X_train.iloc[:5,:]) )
```
...
print(f"accuracy score: {pipe.score(X_test,y_test):.5f}")
```
::::
