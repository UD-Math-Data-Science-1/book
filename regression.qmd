...
```{python}
X_train_reduced = X_train.drop(X.columns[zeroed], axis=1)
X_test_reduced = X_test.drop(X.columns[zeroed], axis=1)
```
...
Instead of using a measure of subset impurities to determine the best split, the split is chosen to cause the greatest reduction in dispersion within the two subsets. We introduce a new dispersion measure here for this purpose.

::::{#def-regression-mad}
If $x_1,\ldots,x_n$ are a sample with median equal to $m$, the **mean absolute deviation (MAD)** of the sample is
$$
\frac{1}{n} \sum_{i=1}^n | x_i - m |.
$$
::::

As with classification, a decision tree regressor seeks the best possible split of samples along coordinate planes. A proposal to split into subsets $S$ and $T$ is assigned the weighted score
$$
Q = |S| H(S) + |T| H(T),
$$
where $H$ is a dispersion measure. The split location is chosen to minimize $Q$. After the process is applied recursively, each leaf of the decision tree contains a subset of the training samples. There are two distinct variations:

1. The mean of the leaf labels is the regression value, and $H$ is the standard deviation.
2. The median of the leaf labels is the regression value, and $H$ is the MAD.
...
The original value set has median $\frac{1}{2}$, so its MAD is 
$$ 
...
Note that $\abs{S}=1$ and $\abs{T}=3$, so
$$ 
\begin{split}
...
Thus, the first split above produces the smallest weighted dispersion, and it improves on the orignal set.
::::
...
We fit a logistic regression just like any other learner. (The meaning of the argument `C` in the call below is explained in @sec-regression-logistic-regular.)

```{python}
from sklearn.linear_model import LogisticRegression

logreg = LogisticRegression(C=10000, solver="liblinear")
logreg.fit(X_train, y_train)
```

::: {.callout-tip}
The use of the `solver` keyword argument in `LogisticRegression` is optional, but the choice used above seems to be far faster and more reliable than the default, for small datasets at least.
:::
...
In @exm-regression-spam, we used $C=10000$, which puts so much weight on the cross-entropy part of the loss function that the regularization is essentially ignored. The default standardization uses the 2-norm with $C=1$, which, in this case, improves the accuracy a bit:
...
$$
where $\alpha$ is a nonnegative constant. When $\alpha = 0$, it's clear that $w=3$ is the minimizer of $r(w)$. Show that if $\alpha>0$, then $r'(w)$ is zero at at most two values of $w$, both of which are less than 3. 
::::
