...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_ewiseksk&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_12y2ruwb%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.0" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_ewshhvaa&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_deld6hwq%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
We say that the model is *parameterized by* $a$ and $b$ in @eq-regression-linear, and we must specify how to use the data to select their values. We describe this process as **fitting** the model to the data, and it almost always means solving an optimization problem.

::::{#def-regression-loss}
A **loss function** is a scalar function of a model's parameters that measures how well a model fits the data. The loss function is minimized to choose the best values for the parameters.
::::
...
::::{#exm-regression-linear chapter=5 type=✍️ description="Linear regression"}
...
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_yhpg0xey&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_phkn41tc%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
</br>
![](_media/memes/slope-intercept.jpg){.lightbox width=200px fig-align="center"}
...
::::{#def-regression-ones}
The **ones vector** $\bfe$ is a vector of length $n$ with all elements equal to 1:
...
Now we can rewrite the loss function in @eq-regression-linear-loss as a vector operation. If we define $\bfx$ as a vector of the $x_i$ and $\bfy$ as a vector of the $y_i$, then

$$
L(a,b) =  \twonorm{a\, \bfx + b \,\bfe - \bfy}^2,
$$ {#eq-regression-linear1d-vector}

::::{#def-regression-least-squares}
The **linear least squares** problem for a sequence of points $(x_i,y_i)$ is to minimize the loss function $L(a,b)$ in @eq-regression-linear1d-vector over all values of $a$ and $b$.
::::

(More specifically, this setup is sometimes called *simple least squares* or *ordinary least squares*.)

:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_z3epf6f9&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_qpriquvr%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.1.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
:::

We can write out the equations @eq-regression-linear1d for $a$ and $b$ more compactly using another major idea from linear algebra.
...
::: {.callout-note}
*Inner product* is a term from linear algebra. In physics and vector calculus with $d=2$ or $d=3$, the same thing is often called a *dot product* or a *scalar product*.
:::

::: {.callout-note}
The ${}^T$ symbol is a *transpose* operation in linear algebra. We won't need it as an independent concept, so we are just using it as notation for the inner product.
:::

The vector inner product is defined only between two vectors of the same length (i.e., dimension). There is an important link between the inner product and the 2-norm:
$$
\bfu^T \bfu = \sum_{i=1}^d u_i^2 = \twonorm{\bfu}^2. 
$$ {#eq-regression-hilbert}

::::{#exm-regression-dot chapter=5 type=✍️ description="Inner product"}
...
In fact, in the next section we will show that the operation on the left-hand side is how we define the product of a matrix and a vector. Thus,
$$
\begin{bmatrix}
\bfx^T \bfx & \bfx^T\bfe \\
\bfe^T \bfx & \bfe^T\bfe
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_myxlnosi&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_0stex25a%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.1.2" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
The MSE is simply $1/n$ times the least-squares loss function $L$. MAE is related but less sensitive than MSE to large outliers. Both quantities are dimensional and therefore depend on how the variables are scaled, but only the units of MAE are the same as of the data.

::::{#exm-regression-mse-mae chapter=5 type=✍️ description="Mean squared and mean absolute error"}
...
::::{#def-regression-cod}
The **coefficient of determination** (CoD) is defined as
$$
...
4. If the $\hat{y}_i$ are found from a linear least-squares regression, then $\text{CoD}$ is the square of the Pearson correlation coefficient between $\bfy$ and $\hat{\bfy}$.

::::

::: {.callout-warning}
Typically the coefficient of determination is denoted $R^2$ because of the special case in part 4 of @thm-regression-cod. However, if the regression method is different, or if the testing set is not equal to the training set, there is no simple relationship between the CoD and the Pearson coefficient. In fact, the CoD can actually be *negative*, so writing it as the square of another number makes no sense in general.

:::

::::{#exm-regression-cod chapter=5 type=✍️ description="Coefficient of determination"}
...
$$
\begin{split}
    \text{CoD} & = 1 - \frac{\left(0+1\right)^2 + \left(2-0\right)^2 + \left(3-1\right)^2}{14/3}  \\ 
    &= 1 - \frac{9}{14/3} = -\frac{13}{14}.
\end{split}
$$

Since this result is negative, we would be better off always predicting $5/3$ rather than using $\hat{f}(x)=x$. (Always predicting the sample mean results in $\text{CoD}=0$, which is better than a negative value.)

::::

::::{#exm-regression-ice-linear chapter=5 type=💻 description="Linear regression for sea ice dataset"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_kxpjms7o&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_p6q1nfgy%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.5" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_jfzjl1ac&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_9x3kymb1%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.2" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

We first observe that we can drop the intercept term $b$ from the discussion, because we could always define an additional constant feature $x_0=1$ and get the same expression. So we will drop $b$ for simplicity, though it is present by default in practice.
...
::: {.callout-note}
Multilinear regression is usually called simply *linear regression* most of the time. What we previously called linear regression, with only a single feature plus intercept, is just a special case. The `LinearRegression` learner class does both types of fits. It has a keyword option `fit_intercept` that determines whether or not the $b$ term in @eq-regression-multilinear is present.
...
The cornerstone of linear algebra is the multiplication of a matrix by a vector. This operation is motivated by mathematical properties that have implications throughout math, science, and engineering, though we will take just the smallest taste.
...
::::{#exm-regression-matvec chapter=5 type=✍️ description="Matrix times vector"}
Suppose that 
$$
\bfX = \begin{bmatrix}
3 & -1 \\ 0 & 2 \\ 1 & 4
...
::::{#exm-regression-cars-multi chapter=5 type=💻 description="Multilinear regression for MPG dataset"}
...
:::{.column-margin}
![](_media/memes/multiple-regression.jpg){.lightbox width=200px fig-align="center"}
:::

In the next example we see that we can create new features out of the ones that are initially given, a tactic known as *feature engineering*. Sometimes these new features add a lot to the quality of the regression. 

::::{#exm-regression-sales-multi chapter=5 type=💻 description="Multilinear regression for sales dataset"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_0otfbu23&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_7e0x0k4l%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.8" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::


It's not uncommon to introduce a product term as done in @exm-regression-sales-multi, and more exotic choices are also possible. Keep in mind, though, that additional variables usually add variance to the model.
...
An important special case of @eq-regression-multilinear happens when all of the features $x_i$ are powers of a single variable $t$: 
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_3228tsmj&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_u9jh73jh%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.2.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

This turns the regressive approximation into

$$
y \approx w_1 + w_2 t + \cdots + w_d t^{d-1},
$$ {#eq-regression-polynomial}

which is a polynomial of degree $d-1$ in the $t$ variable. This allows representation of data that depends on $t$ in ways more complicated than a straight line. However, it can lead to overfitting if taken too far.

::: {.callout-note}
It might seem odd to categorize polynomial fits under the heading "linear." The $y$ in @eq-regression-polynomial has a nonlinear dependence as a function of $t$. But the dependence on the weights $\bfw$ is linear, which means that the problem can be solved by linear algebra methods alone.

:::

Rather than constructing polynomial features manually, it's easier to use a pipeline.

::::{#exm-regression-polynomial chapter=5 type=💻 description="Polynomial regression for the MPG dataset"}
We return to the data set regarding the fuel efficiency of cars:
...
cubic = Pipeline((
    ("poly", PolynomialFeatures(degree=3)), ("linear", lm)
    ))
cubic.fit(X, y)
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_myfj2re4&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_d8fyw2rp%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.9" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
::::{#exm-regression-polynomial-high chapter=5 type=💻 description="Overfitting in polynomial regression"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_jyjb5url&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_6b2oymip%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.10" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_i48yjeqq&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_5mv99q3b%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.3" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
![](_media/memes/penalty.jpg){.lightbox width=200px fig-align="center"}
:::

The new terms in @eq-regression-ridge and @eq-regression-ridge based on norms of $\bfw$ are called **penalty terms**. As $\alpha\to 0$, the penalty terms vanish and both types of regularization revert to the usual least-squares loss. But as $\alpha \to \infty$, the penalty terms dominate and the optimization becomes increasingly concerned with prioritizing a small result for $\bfw$. In this limit, the model pays less and less attention to the data, which can reduce overfitting.

::::{#exm-regression-ridge-diabetes chapter=5 type=💻 description="Ridge regression for diabetes dataset"}
We'll apply ridge regression to data collected about the progression of diabetes:
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_cjwkntgp&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_yvxzucyg%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.11" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

While ridge regression is an easier function to minimize quickly, LASSO has an interesting advantage, as illustrated in this figure for two features:

![](_media/lasso.svg)

On the left, the elliptical contours represent the unregularized, data-fitting loss function. The best we could do is to be at the blue dot, but regularization changes the result. The points lying on a circle centered at the origin make a constant contribution to the ridge regularization loss term. You can see why the point on such a circle that gets the smallest fitting loss is where a loss contour just grazes the circle tangentially. Whichever circle ultimately ends up being best, the optimum point will lie on a point of tangency like this.

The situation on the right for LASSO shows an important difference: the constant contributions to fitting loss are on "circles" of constant 1-norm that are actually rotated squares. It's intuitively clear from the picture that the point on this square minimizing the fitting part of the loss is likely to be a corner. 

The idea shown here scales up into higher dimensions—the minimizers in LASSO tend to be corner points at which some of the weights are exactly zero, producing so-called *sparse* weight vectors. These zeros indicate features that have low predictive value. Those variables can often be removed from the regression to reduce model variance without significantly affecting the bias.

::::{#exm-regression-lasso-diabetes chapter=5 type=💻 description="LASSO regression for diabetes dataset"}
We continue with the diabetes data from @exm-regression-ridge-diabetes, but this time, we try LASSO regularization. A validation curve suggests initial gains in the CoD score as the regularization parameter $\alpha$ is varied, followed by a decrease:
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_3kg9kwjn&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_eoj7cms0%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.12" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_tgywzwbw&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_eygv9048%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.4" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
::::{#exm-regression-linear-2 chapter=5 type=✍️ description="Proving linearity"}
...
::::{#exm-regression-nonlinear chapter=5 type=✍️ description="Proving nonlinearity"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_gcf5bxmt&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_u15xbse4%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.4.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

::::{#exm-regression-knn chapter=5 type=✍️ description="Nonlinearity of kNN regression"}
...
::::{#exm-regression-knn-demo chapter=5 type=💻 description="kNN regression for MPG dataset"}
...
model = Pipeline(( 
    ("z", StandardScaler()), ("knn", KNeighborsRegressor())
    ))
grid = {
    "knn__n_neighbors": range(2, 25),
    "knn__weights": ["uniform", "distance"] 
    }
optim = GridSearchCV(
    model, grid, cv=kf, n_jobs=-1
    )
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_vcdc7ddk&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_j12g6hic%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.14" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
</br>
![](_media/memes/not-a-classifier.jpg){.lightbox width=200px fig-align="center"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_5zlo1g14&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_p72p0z0c%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.4.2" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
::::{#exm-regression-split chapter=5 type=✍️ description="Creating a decision tree regressor"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_pjw3mdbc&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_emr6qjtf%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.15" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

::::{#exm-regression-tree chapter=5 type=💻 description="Tree regression on a small dataset"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_bpcrc2rs&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_3ndmgr5x%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.16" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

::::{#exm-regression-tree-demo chapter=5 type=💻 description="Random forest for regression"}
...
randForest = RandomForestRegressor(n_estimators=60)
optim = GridSearchCV(
    randForest, grid, cv=kf, n_jobs=-1
    )
optim.fit(X_train, y_train)
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_8ioj8704&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_sujcidya%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.5" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_o2jgpf6u&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_828gz2j5%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Section 5.5.1" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>

:::

However, it's more common to use a different kind of loss function.

::::{#def-regression-cross-entropy}
For a logistic model with predictions $\hat{p}_i = \sigma(\bfx_i^T\bfw)$, the **cross-entropy** loss function is
...
::: {.callout-note}
The logarithms in @eq-regression-cross-entropy can be in any base, since that choice only affects $L$ by a constant factor. 

:::

Suppose we choose some $i$ for which $y_i=0$. Then the contribution of the $i$th term in the sum of @eq-regression-cross-entropy is
...
::::{#exm-regression-cross-entropy chapter=5 type=✍️ description="Cross-entropy"}
...
:::{.column-margin}
![](_media/memes/cross-entropy.jpeg){.lightbox width=200px fig-align="center"}
:::
...
::::{#exm-regression-spam chapter=5 type=💻 description="Logistic regression as a spam filter"}
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_d2b8yjqe&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_83hqxz1r%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.19" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
::::{#exm-regression-spam-regular chapter=5 type=💻 description="Regularization for the spam filter regressor"}
...
grid = { "logistic__C": 10 ** np.linspace(-1, 4, 40), 
...
:::{.column-margin}
<div style="max-width:304px"><div style="position:relative;padding-bottom:75.986842105263%"><iframe id="kaltura_player" src="https://cdnapisec.kaltura.com/p/2358381/embedPlaykitJs/uiconf_id/57659783?iframeembed=true&amp;entry_id=1_y9qykrqv&amp;config%5Bprovider%5D=%7B%22widgetId%22%3A%221_rjio0tvc%22%7D&amp;config%5Bplayback%5D=%7B%22startTime%22%3A0%7D" width="304" height="231" allowfullscreen webkitallowfullscreen mozAllowFullScreen allow="autoplay *; fullscreen *; encrypted-media *" sandbox="allow-downloads allow-forms allow-same-origin allow-scripts allow-top-navigation allow-pointer-lock allow-popups allow-modals allow-orientation-lock allow-popups-to-escape-sandbox allow-presentation allow-top-navigation-by-user-activation" frameborder="0" title="Example 5.20" style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div></div>
...
::::{#exm-regression-logistic-ovr chapter=5 type=✍️ description="Multiclass logistic regression"}
...
::::{#exm-regression-logistic-gas chapter=5 type=💻 description="Multiclass logistic regression for forest cover dataset"}
...
pipe = Pipeline((
    ("z", StandardScaler()), ("logistic", logr)
    ))
pipe.fit(X_train, y_train)
...
(5.2) Suppose that values $y_i$ for $i=1,\ldots,n$ are to be fit to 2D sample vectors using a multilinear regression function $\hat{f}(\bfx)=w_1 x_1 + w_2 x_2$. Define the sum-of-squared-residuals loss function
$$ 
L(w_1,w_2) = \sum_{i=1}^n \bigl(y_i - \hat{f}(\bfx_i)\bigr)^2. 
$$
...
**(a)** Show that if $w < 0$ and $\alpha>0$, then $L'(w)$ can never be zero. (Remember that for such $w$, $|w|=-w$.)

**(b)** Show that if $w>0$ and $0<\alpha < 12$, then $L'(w)$ has a single root in the interval $(0,3)$.
::::
...
(5.4) Given the data set $(x_i,y_i)=\{(0,-1),(1,1),(2,3),(3,0),(4,3)\}$, find the MAE-based $Q$ score for the following hypothetical decision tree splits.
...
(5.5) Let $\bfX=[[-1],[0],[1]]$ and $\bfy=[0,1,0]$. This small dataset is to be fit to a probabilistic predictor $\hat{p}(x) = \sigma(w x)$ for scalar weight $w$. 
