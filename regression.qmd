...
Time series have a structure that allows some particular types of analyses: namely, that the past and present can affect the future.

A simple way to exploit this structure is **autoregression**. To use it, we have to make a standard assumption that the time series is *stationary*. A formal definition is beyond us here, but in words, stationarity means that the statistical properties of the time series do not change over time. As a crude example, it might be justified to view the readings of a self-driving car as stationary during rural freeway driving in clear weather, but not as the car makes a transition to city streets or a thunderstorm.

```{python}
ghcn = pd.read_csv("ghcn_newark.csv", date_parser="datetime", index_col=1)
ghcn.head()
```

```{python}
sns.relplot(data=ghcn["1980"], 
    x="DATE", y="TMIN",
    kind="line"
    )
```
...
```{python}
ghcn["Tdev"] = ghcn.groupby(ghcn.index.month)["TMIN"].transform(lambda x: x - x.mean())
sns.relplot(data=ghcn["1980"], 
    x="DATE", y="Tdev",
    kind="line"
    )
...
```{python}
yr = pd.date_range("1990-01-01", "1990-12-31", freq="D")

def lag(x, n):
    return x[yr - pd.DateOffset(days=n)].values
...
names = []
for l in range(1,5):
    cols["lag"+str(l)] = lag(ghcn["Tdev"],l)
